...
- [ ] **Smarter pick**: learned detector (YOLO) + point-cloud + shape-completion (vs colour + plane). *In progress:* the work camera's depth is now back-projected to a coloured point cloud in the twin (v0.7.10, `PCL` toggle); next is a learned detector + grasp synthesis on the cloud.
